...
// frequency limits
const uint32_t freqLL=400;
const uint32_t freqLH=800;
const uint32_t freqHL=1800;
const uint32_t freqHH=2300;

const uint8_t blinker = 13;                                   // sqelch output chanell 
const uint8_t adcChan = 0;                                    // audio input on A0
...
const uint8_t binLL=(freqLL * samples / samplingFrequency)+1;
const uint8_t binLH=(freqLH * samples / samplingFrequency)+1;
const uint8_t binHL=(freqHL * samples / samplingFrequency)+1;
const uint8_t binHH=(freqHH * samples / samplingFrequency)+1;
...
int16_t sqlStatus=0;
uint16_t count = 0;
char p[17];     //itoa() conversion buffer
uint8_t q[17];  //itoa() conversion buffer
...
void initialize_ADC( uint8_t adcChan ) // !!!МОЖНО ПЕРЕВЕСТИ НА ИСПОЛЬЗОВАНИЕ TIMER0!!!
{
  cli();
...
void loop()
{
  ...
  if ( adcFlag )
  {
    ...
    UART_SendArray(itoa(powerLSS,p,10));
    UART_SendByte(' ');
    UART_SendArray(itoa(powerHSS,p,10));
    UART_SendByte(' ');
    UART_SendArray(itoa(sqlStatus,p,10));
    UART_SendByte('\n');
